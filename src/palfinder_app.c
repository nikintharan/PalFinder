...
static void bg_update_proc(Layer *layer, GContext *ctx) {
  ...
  PalfinderAppMainWindowViewModel *model = &data->view_model;
...
static void set_data_point(PalfinderAppData *data, PalfinderAppDataPoint *dp) {
  data->data_point = dp;
  palfinder_app_view_model_fill_all(&data->view_model, dp);
}

static void view_model_changed(struct PalfinderAppMainWindowViewModel *arg) {
  PalfinderAppMainWindowViewModel *model = (PalfinderAppMainWindowViewModel *)arg;
...
static void main_window_load(Window *window) {
  PalfinderAppData *data = window_get_user_data(window);
  ...
  palfinder_app_main_window_view_model_announce_changed(&data->view_model);
}
...
static void after_scroll_swap_text(Animation *animation, bool finished, void *context) {
  PalfinderAppData *data = window_get_user_data(s_main_window);
  PalfinderAppDataPoint *data_point = context;

  palfinder_app_view_model_fill_strings_and_pagination(&data->view_model, data_point);
}
...
static Animation *animation_for_scroll(PalfinderAppData *data, ScrollDirection direction, PalfinderAppDataPoint *next_data_point) {
  PalfinderAppMainWindowViewModel *view_model = &data->view_model;
  ...
  Animation *bg_animation = palfinder_app_create_view_model_animation_bgcolor(view_model, next_data_point);
  ...
  // morphing icon
  Animation *icon_animations = palfinder_app_create_view_model_animation_icon(view_model, next_data_point, BACKGROUND_SCROLL_DURATION * 2);

  // changing temperature text
  Animation *number_animation = palfinder_app_create_view_model_animation_numbers(view_model, next_data_point);
...
static Animation *animation_for_bounce(PalfinderAppData *data, ScrollDirection direction) {
  return create_inbound_anim(data, direction);
}

static void ask_for_scroll(PalfinderAppData *data, ScrollDirection direction) {
  int delta = direction == ScrollDirectionUp ? -1 : +1;
  PalfinderAppDataPoint *next_data_point = palfinder_app_data_point_delta(data->data_point, delta);
...
static void init() {
  PalfinderAppData *data = malloc(sizeof(PalfinderAppData));
  memset(data, 0, sizeof(PalfinderAppData));

  PalfinderAppDataPoint *dp = palfinder_app_data_point_at(0);
